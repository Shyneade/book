...
def donation():
    ...
    if request.method == 'GET':
        ...
        don = Donation(don_amt=amount,don_userid=userid,don_fullname=fullname,don_email=email,
        don_refno=refno,don_status='pending')
        db.session.add(don)
        db.session.commit()
        #save the refno in a session so that we can retrieve the details on the nxt pg.
        session['ref'] = refno
...
def make_payment():
    ...
    if session.get('ref') !=None:
        ref = session['ref']
        #To do: we want to get the details o the transaction and display to the user
        trxdeets = db.session.query(Donation).filter(Donation.don_refno==ref).first()
        return render_template("user/payment.html", trxdeets=trxdeets) 
    else:
...
def paystack():
    if session.get('ref') != None:
        ...
        "Authorization":"Bearer pk_test_5fe46517d611c38c831ae78811c80f0ca6281d6f"}
        data = {"email": email, "amount":amount*100, "reference":ref}
        response = requests.post(url, headers=headers, data=json.dumps(data))
        rspjson = response.json()    #this is leg one of paystack response
...
def paystack_landing():
    ...
    if ref == None:
        ...
        rspjson = json.loads(response.text)    #this is leg one of paystack response
        return rspjson
...
def home():
    ...
    useronline = session.get('userid')    
    userdeets = db.session.query(User).get(useronline)
    #we will connect to the endpoint in order to fetch the stores >>>http//127.0.0.1/api/v1.0/listall
    headers = {'Content-Type':"application/json"}
